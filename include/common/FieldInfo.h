...
class EnumInfo {
   public:
    ...
    void resize(size_t val) { enums_.resize(val); }
    ...
    const std::vector<std::pair<std::string, Value>> &getEnums() const { return enums_; }
 ...
 */
class FieldInfo {  // TODO: check for atavism ?
   public:
...
class InfoList : public std::vector<FieldInfo *> {  // TODO: check for atavism ?
   public:
