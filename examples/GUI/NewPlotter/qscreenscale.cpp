...
QScreenScale::QScreenScale(const int &index0, const QSizeF &sz, const LineProperties &dstx,
                           const Margin &margin, QWidget *parent)
{
    Q_UNUSED(parent)
...
//-------------------------------------------------------------------------
int QScreenScale::getIndex() const
{
    return index_;
}
//-------------------------------------------------------------------------
...
//-------------------------------------------------------------------------
void QScreenScale::setScale(QVector<RelativeTime> *xs)
{
    p_scale_ = xs;
}
//-------------------------------------------------------------------------
...
//-------------------------------------------------------------------------
void QScreenScale::setMargins(const Margin &margin)
{
    margin_ = margin;
}
//-------------------------------------------------------------------------
void QScreenScale::advance(int phase)
{
    Q_UNUSED(phase)
}
//-------------------------------------------------------------------------
int QScreenScale::type() const
{
    return type_;
}
//-------------------------------------------------------------------------
    ...
    while (c_ptr) {
        ...
        for (size_t i = 0; i < c_ptr->number_of_points_; i++){
            ...
            for(auto lvl : levels_){
                if(lvl.cross){
                    if(dot.val_max >= lvl.value)
                        dot.warning = true;
                }
                else {
                    if(dot.val_min <= lvl.value)
                        dot.warning = true;
                }
            }
...
void QScreenScale::drawDiag()
{
    ...
    dt.ms_fractional = 0;
    int freshIndex = image_w * (dt.toDouble() - t_step_.toDouble()) / dt.toDouble();
    ...
    if(!minimum_.automatic)
        scale_min = minimum_.value;

    double      scale_max = stat_.val_max;
    if(!maximum_.automatic)
        scale_max = maximum_.value;
...
void QScreenScale::drawScale()
{
    ...
    if (maximum_.automatic)
        val_max = stat_.val_max; //prm->d_out.valMax;
    else
        val_max = maximum_.value;

    if (minimum_.automatic)
        val_min = stat_.val_min; //prm->d_out.valMin;
    else
...
//-------------------------------------------------------------------------
void QScreenScale::onSettingsShow()
{
    emit toTuned(type_, index_);
}
//-------------------------------------------------------------------------
...
//-------------------------------------------------------------------------
void QScreenScale::onSetPause(const bool is_paused)
{
...
void QScreenScale::changeScaleBorder(const bool &high, const int &delta)
{
    ...
    int pabs = 0;
    if (vabs > 0)
        pabs = static_cast<int>(floor(log10(vabs) + 0.5));

    qreal rng = maximum_.value - minimum_.value;
    int prng = 0;
    if (rng > 0)
        prng = static_cast<int>(floor(log10(rng)));
...
//-------------------------------------------------------------------------
void QScreenScale::onAlignT()
{
    emit toAlign(type_, index_, 0);
}
//-------------------------------------------------------------------------
void QScreenScale::onAlignB()
{
    emit toAlign(type_, index_, 1);
}
//-------------------------------------------------------------------------
void QScreenScale::onAlignL()
{
    emit toAlign(type_, index_, 2);
}
//-------------------------------------------------------------------------
void QScreenScale::onAlignR()
{
    emit toAlign(type_, index_, 3);
}
//-------------------------------------------------------------------------
void QScreenScale::onAlignH()
{
    emit toAlign(type_, index_, 4);
}
//-------------------------------------------------------------------------
...
void QScreenScale::wheelEvent(QGraphicsSceneWheelEvent *event)
{
    ...
    if (event->modifiers() & Qt::ShiftModifier) {
        ...
        if (event->delta() > 0) {
            h += 20;
            if (h > AXIS_Y_HEIGHT_MAX)
                h = AXIS_Y_HEIGHT_MAX;
        } else {
            h -= 20;
            if (h < AXIS_Y_HEIGHT_MIN)
                h = AXIS_Y_HEIGHT_MIN;
        }
        r.setHeight(h);
        setRect(r);
...
//-------------------------------------------------------------------------
const QList<uint32_t>  QScreenScale::DIAG_COLORS = {
        0xEF5350, 0xF44336, 0xE53935, 0xD32F2F, 0xC62828, 0xB71C1C,
        0xEC407A, 0xE91E63, 0xD81B60, 0xC2185B, 0xAD1457, 0x880E4F,
        0xAB47BC, 0x9C27B0, 0x8E24AA, 0x7B1FA2, 0x6A1B9A, 0x4A148C,
        0x7E57C2, 0x673AB7, 0x5E35B1, 0x512DA8, 0x4527A0, 0x311B92,
        0x5C6BC0, 0x3F51B5, 0x3949AB, 0x303F9F, 0x283593, 0x1A237E,
        0x42A5F5, 0x2196F3, 0x1E88E5, 0x1976D2, 0x1565C0, 0x0D47A1,
        0x29B6F6, 0x03A9F4, 0x039BE5, 0x0288D1, 0x0277BD, 0x01579B,
        0x26C6DA, 0x00BCD4, 0x00ACC1, 0x0097A7, 0x00838F, 0x006064,
        0x26A69A, 0x009688, 0x00897B, 0x00796B, 0x00695C, 0x004D40,
        0x66BB6A, 0x4CAF50, 0x43A047, 0x388E3C, 0x2E7D32, 0x1B5E20,
        0x9CCC65, 0x8BC34A, 0x7CB342, 0x689F38, 0x558B2F, 0x33691E,
        0xD4E157, 0xCDDC39, 0xC0CA33, 0xAFB42B, 0x9E9D24, 0x827717,
        /*0xFFEE58, 0xFFEB3B, 0xFDD835*/0xFBC02D, 0xF9A825, 0xF57F17,
        /*0xFFCA28, 0xFFC107*/0xFFB300, 0xFFA000, 0xFF8F00, 0xFF6F00,
        0xFFA726, 0xFF9800, 0xFB8C00, 0xF57C00, 0xEF6C00, 0xE65100,
        0xFF7043, 0xFF5722, 0xF4511E, 0xE64A19, 0xD84315, 0xBF360C
};
